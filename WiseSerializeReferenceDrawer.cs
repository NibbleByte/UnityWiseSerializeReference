...
{
	/// <summary>
	/// By default [SerializeReference] fields display empty data with no UI to populate it, so you can't do anything with it.
	/// Use this attribute next to [SerializeReference] to have a "+" button drawn next to the field.
	...
	///
	/// If you want to customize the drawer or have this behaviour everywhere, not just the given field (per type, not per field)
	/// have a property drawer editor that inherits <see cref="WiseSerializeReferenceBasePropertyDrawer"/>
	///
	/// ===== Example =====
	/// [SerializeReference]
	...
	/// public class MyClassDrawer : WiseSerializeReferenceBasePropertyDrawer
	/// {
	/// }
	///
	/// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	///
	/// </summary>
	[CustomPropertyDrawer(typeof(WiseSerializeReferenceDrawerAttribute))]
	public class WiseSerializeReferenceBasePropertyDrawer : PropertyDrawer
	{
		...
		protected void DrawManagedTypeLabel(Rect typeLabelRect, SerializedProperty property, Color color, GUIStyle style = null)
		{
			...
			// managedReferenceFullTypename provides the simple assembly name and type in this format (notice the space):
			// "DevLocker.Audio DevLocker.Audio.AudioPlayerAsset/AudioConductor"

			// If missing, start index is 0, so it's ok.
			int assemblyIndex = property.managedReferenceFullTypename.IndexOf(" ") + 1;

			int typeIndex = property.managedReferenceFullTypename.LastIndexOf(".");
			if (typeIndex == -1) {
				// In case there is no namespace, just start after the assembly.
				typeIndex = assemblyIndex;
			} else {
				typeIndex++; // Start should be after the dot.
			}
	...
	/// <summary>
	/// When you use [SerializeReference] on a list, if user adds elements via "Duplicate Array Element" context menu, the result will be duplicating the reference, not the instance.
	...
	/// By Moe_Baker
	/// </summary>
	public static class WiseSerializeReferenceValidation
	{
		/// <summary>
		...
		private static object DuplicateReference(object original)
		{
			...
			// In other words, we will get all the values we are expecting
